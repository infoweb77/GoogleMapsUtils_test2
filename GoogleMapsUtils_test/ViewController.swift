...
class ViewController: UIViewController, GClusterManagerDelegate, GMSMapViewDelegate {
    ...
    func mapView(mapView: GMSMapView, didTapMarker marker: GMSMarker) -> Bool {
        if let count = marker.userData as? Int {
            NSLog("Did tap marker for cluster item \(count)")
            let newCamera = GMSCameraPosition.cameraWithTarget(marker.position, zoom: mapView.camera.zoom + 1)
